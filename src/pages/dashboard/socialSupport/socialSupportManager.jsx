...
const SocialSupportManager = () => {
	...
	const handleDelete = async ({ _id, title }) => {
		try {
			const response = await del(`social-support/${_id}`);
			refetch();
	...
	const handlePublished = async ({ _id, isPublished }) => {
		try {
			...
			await put(`social-support/update-social-support/${_id}`, { isPublished: updatedIsPublished });
	...
	return (
		<div className="min-h-screen">
			...
			<HeaderText>Social Support Manage</HeaderText>

			{socailSupportData.length > 0 ? (
				<>
					<div className="flex justify-center md:justify-end py-3">
						<Link to="/dashboard/social-support/create-social-support">
							<IconButton>Create Social Support</IconButton>
						</Link>
					</div>
					<div>
								...
								<tbody className="text-color-primary">
									{!isLoading &&
										socailSupportData.map(
											({ _id, title, description, isPublished, image }, index) => (
												<tr key={_id} className="even:bg-color-secondary text-center">
													...
													<td className="p-2 ">
														<div className="flex gap-3 justify-center items-center">
															...
															<Link to={`update-social-support/${_id}`}>
																<Button
																	variant="outlined"
																	size="sm"
						...
						/>
						<ConfirmationModal
							message={`Warning: Deleting this Social support data is permanent and cannot be undone.`}
				...
				<div className="flex flex-col gap-3 justify-center items-center min-h-screen">
					<p className="text-xl text-color-primary">
						No content found please create social support data to start
					</p>
					<Link to="/dashboard/social-support/create-social-support">
						<IconButton>Create Social Support</IconButton>
					</Link>
...
export default SocialSupportManager;
